...
// 依照購買時的rand大小取出陣列
if($error=="0"){
	$sql="select * from itemList03 where relaId1=".$_POST['relaId1']." order by rand1 DESC";
	$data1=getSql($sql);
	...
	for($i=0;$i<count($data1);$i++){
		$sql="update itemList03 set relaId2=".$item[$i]." where systemId=".$data1[$i]['systemId'];
		doSql($sql);
	}
}
// 依照購買時的rand大小取出陣列

// 隨機總抽數用陣列位置發給隨機到的陣列位置
if($error=="0"){
	// print_r(count($data1));
	// 陣列總數
	$ary=count($data1)-1;
	...
	$sql="update itemList set last2=".$data1[$rand]['userId1']." where systemId=".$_POST['relaId1'];
	doSql($sql);
